...
chunk_t *newline_add_before(chunk_t *pc)
{
   ...
   retval_if(chunk_is_newline(prev), prev);
...
chunk_t *newline_add_after(chunk_t *pc)
{
   ...
   retval_if(chunk_is_newline(next), next);
...
chunk_t *newline_add_between(chunk_t *start, chunk_t *end)
{
   ...
   {
      retval_if(chunk_is_newline(pc), pc);
   }
...
static bool newlines_if_for_while_switch(chunk_t *start, argval_t nl_opt)
{
   ...
   if (is_type(pc, CT_SPAREN_OPEN))
   {
      ...
      if (is_type  (brace_open, 2, CT_BRACE_OPEN, CT_VBRACE_OPEN) &&
...
static chunk_t *get_closing_brace(chunk_t *start)
{
   ...
   {
      if (is_type(pc, 2, CT_BRACE_CLOSE, CT_VBRACE_CLOSE) &&
          (pc->level == level))
      {
         return(pc);
      }
...
static void newlines_if_for_while_switch_post_blank_lines(chunk_t *start, argval_t nl_opt)
{
   ...
   if (is_type(start, CT_IF))
   {
      while (true)
      {
         ...
         if (is_type(next, 2, CT_ELSE, CT_ELSEIF))
         {
...
static void newlines_struct_enum_union(chunk_t *start, argval_t nl_opt, bool leave_trailing)
{
           ...
           (pc->level >= level))
   {
      break_if((pc->level == level) &&
                is_type(pc, 4, CT_VSEMICOLON, CT_BRACE_OPEN,
...
static void newlines_enum(chunk_t *start)
{
   ...
   {
      break_if ((pc->level == level) &&
                 is_type(pc, 4, CT_BRACE_OPEN, CT_ASSIGN,
   ...
   if (is_type(pc, CT_BRACE_OPEN))
   {
      ...
      nl_opt = (!chunk_is_comment_or_newline(next)) ?
            nl_opt = AV_IGNORE :
...
static void newlines_do_else(chunk_t *start, argval_t nl_opt)
{
   ...
   if (is_type(next, 2, CT_BRACE_OPEN, CT_VBRACE_OPEN))
   {
         ...
         if (is_type(pc, CT_TYPEDEF))
         {
            ...
            var_blk       = false;
                  ...
                  ((chunk_is_var_type(next) ||
                    is_type(next, 2, CT_WORD, CT_FUNC_CTOR_VAR))) &&
...
static void newline_case(chunk_t *start)
{
   ...
   do  // \todo replace with chunk_search
   {
      ...
      return_if((chunk_is_newline(prev)) &&
                (prev->nl_count > 1    ) );
   ...
   if (is_type(prev, 2, CT_SEMICOLON, CT_BRACE_CLOSE))
   {
      if (chunk_is_newline(pc) &&
         (pc->nl_count < 2   ) )
      {
...
static void newline_before_return(chunk_t *start)
{
   ...
   assert(is_valid(pc));
   return_if(is_type(pc, 2, CT_BRACE_OPEN, CT_VBRACE_OPEN));

   if (chunk_is_comment(pc))
   {
      pc = chunk_get_prev(pc);
      assert(is_valid(pc));
      return_if(!chunk_is_newline(pc));
      nl = pc;
   }
   if (nl->nl_count < 2)
   {
...
static void newline_after_return(chunk_t *start)
{
   ...
   return_if(is_type(after, 3, CT_BRACE_CLOSE, CT_VBRACE_CLOSE, CT_ELSE));
...
static bool one_liner_nl_ok(chunk_t *pc)
{
   ...
   {
      const c_token_t type = is_type(br_open, CT_BRACE_CLOSE) ? CT_BRACE_OPEN : CT_VBRACE_OPEN;
   ...
   if (is_valid(pc)          &&
       ...
       is_type(pc, 4, CT_BRACE_OPEN, CT_BRACE_CLOSE, CT_VBRACE_OPEN, CT_VBRACE_CLOSE))
   {
      // \todo DRY
      ...
      if (cpd.settings[UO_nl_assign_leave_one_liners].b && (pc->ptype == CT_ASSIGN))
      {
         LOG_FMT(LNL1LINE, "false (assign)\n");
         return(false);
      }

      if (cpd.settings[UO_nl_enum_leave_one_liners  ].b && (pc->ptype == CT_ENUM))
      {
         LOG_FMT(LNL1LINE, "false (enum)\n");
         return(false);
      }

      if (cpd.settings[UO_nl_getset_leave_one_liners].b && (pc->ptype == CT_GETSET))
      {
      ...
      if (cpd.settings[UO_nl_func_leave_one_liners].b && (pc->ptype == CT_OC_MSG_DECL))
      {
         LOG_FMT(LNL1LINE, "false (method def)\n");
         return(false);
      }

      if (cpd.settings[UO_nl_cpp_lambda_leave_one_liners].b && ((pc->ptype == CT_CPP_LAMBDA)))
      {
      ...
      if (cpd.settings[UO_nl_while_leave_one_liners].b && (pc->ptype == CT_WHILE))
      {
...
void newlines_cleanup_braces(bool first)
{
   ...
   for ( ; is_valid(pc); pc = chunk_get_next_ncnl(pc))
   {
      ...
      case(CT_VBRACE_OPEN):
         ...
         if ((((pc->ptype == CT_IF    ) ||
               (pc->ptype == CT_ELSEIF) ||
               (pc->ptype == CT_ELSE  ) ) && cpd.settings[UO_nl_create_if_one_liner   ].b) ||
              ((pc->ptype == CT_FOR   )   && cpd.settings[UO_nl_create_for_one_liner  ].b) ||
              ((pc->ptype == CT_WHILE )   && cpd.settings[UO_nl_create_while_one_liner].b) )
         {
            nl_create_one_liner(pc);
         }
         if ((((pc->ptype == CT_IF    ) ||
               (pc->ptype == CT_ELSEIF) ||
               (pc->ptype == CT_ELSE  ) ) && cpd.settings[UO_nl_split_if_one_liner   ].b) ||
              ((pc->ptype == CT_FOR   )   && cpd.settings[UO_nl_split_for_one_liner  ].b) ||
              ((pc->ptype == CT_WHILE )   && cpd.settings[UO_nl_split_while_one_liner].b) )
         {
            if (pc->flags & PCF_ONE_LINER)
            {
               // split one-liner
...
static void nl_handle_define(chunk_t *pc)
{
   ...
   {
      if (is_type(nl, CT_NEWLINE)) { return; }
...
void newline_after_multiline_comment(void)
{
   ...
   {
      if (is_not_type(pc, CT_COMMENT_MULTI)) { continue; }
...
void newline_after_label_colon(void)
{
   ...
   for (chunk_t *pc = chunk_get_head(); is_valid(pc); pc = chunk_get_next(pc))
   {
      if (is_not_type(pc, CT_LABEL_COLON)) { continue; }
      newline_add_after(pc);
   }
}
...
void newlines_squeeze_ifdef(void)
{
   ...
   for (pc = chunk_get_head(); is_valid(pc); pc = chunk_get_next_ncnl(pc))
   {
          ...
          cpd.settings[UO_nl_squeeze_ifdef_top_level].b))
      {
         ...
         if (is_type(ppr, 3, CT_PP_IF, CT_PP_ELSE, CT_PP_ENDIF) )
         {
            chunk_t *pnl = nullptr;
            chunk_t *nnl = chunk_get_next_nl(ppr);
            if (is_type(ppr, 2, CT_PP_ELSE, CT_PP_ENDIF))
            {
            ...
            if (is_valid(nnl))
            {
               ...
               if (is_type(ppr, 2, CT_PP_IF, CT_PP_ELSE))
               {
                  if (nnl->nl_count > 1)
                  {
...
void newlines_class_colon_pos(c_token_t tok)
{
   ...
   for (chunk_t *pc = chunk_get_head(); is_valid(pc); pc = chunk_get_next_ncnl(pc))
   {
      ...
      {
         if (is_type(pc, 2, CT_BRACE_OPEN, CT_SEMICOLON ))
         {
            ccolon = nullptr;
            continue;
         }
...
void do_blank_lines(void)
{
   ...
   for (chunk_t *pc = chunk_get_head(); is_valid(pc); pc = chunk_get_next(pc))
   {
      if (is_not_type(pc, CT_NEWLINE)) { continue; }
          ...
          is_type (next, CT_COMMENT_MULTI) )
      {
         ...
         if (is_invalid (prev                                  ) ||
             is_not_type(prev, 2, CT_BRACE_OPEN, CT_VBRACE_OPEN) )
         {
      ...
      if ((cpd.settings[UO_nl_comment_func_def].u != 0)   &&
          ...
          is_ptype         (next, 2, CT_FUNC_DEF,   CT_FUNC_CLASS_DEF) )
      {
           ...
           are_valid(prev, next))
      {
         if ( is_ptype   (prev,    CT_GETSET                   ) &&
              ...
              is_type    (prev, 2, CT_BRACE_CLOSE, CT_SEMICOLON) )
         {
