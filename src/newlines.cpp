...
static void newlines_func_pre_blank_lines(chunk_t *start)
{
   ...
   chunk_t       *last_nl      = nullptr;
   const chunk_t *last_comment = nullptr;
   bool          do_it         = false;
   chunk_t       *pc;
...
static void newlines_brace_pair(chunk_t *br_open)
{
   ...
   if (chunk_is_parent_type(br_open, CT_ASSIGN))
   {
   ...
   if (chunk_is_parent_type(br_open, 8, CT_FUNC_DEF, CT_FUNC_CALL,
         ...
         CT_FUNC_CLASS_DEF, CT_OC_MSG_DECL, CT_CS_PROPERTY) )
   {
      ...
      val = ((chunk_is_parent_type(br_open, 4, CT_FUNC_DEF,
   ...
   if (chunk_is_parent_type(br_open, 2, CT_CLASS, CT_STRUCT))
   {
...
static void newline_func_def(chunk_t *start)
{
   ...
   chunk_t  *prev;
   bool     is_def = chunk_is_parent_type(start, 2, CT_FUNC_DEF, CT_FUNC_CLASS_DEF);
...
void newlines_cleanup_braces(bool first)
{
   ...
   for ( ; chunk_is_valid(pc); pc = chunk_get_next_ncnl(pc))
   {
      ...
      else if (pc->type == CT_WHILE_OF_DO) { newlines_cuddle_uncuddle(pc, cpd.settings[UO_nl_brace_while].a); }
