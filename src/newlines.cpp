...
static void newlines_func_pre_blank_lines(chunk_t *start)
{
   ...
   /*  look backwards until we find open brace (don't add or remove)
   ...
   for (pc = chunk_get_prev(start); is_valid(pc); pc = chunk_get_prev(pc))
   {
      ...
      switch(pc->type)
      {
         ...
         case(CT_DESTRUCTOR): log_msg2(pc, "CT_DESTRUCTOR");      break;
         case(CT_TYPE      ): log_msg3(pc, "CT_TYPE"      );      break;
         case(CT_QUALIFIER ): log_msg2(pc, "CT_QUALIFIER" );      break;
         case(CT_PTR_TYPE  ): log_msg2(pc, "CT_PTR_TYPE"  );      break;
         case(CT_DC_MEMBER ): log_msg3(pc, "CT_DC_MEMBER" );      break;
         default:             log_msg4(pc, "else"); do_it = true; break;
      }
   }
end_of_loop:
#endif
...
static bool one_liner_nl_ok(chunk_t *pc)
{
   ...
   if (is_valid(pc)          &&
       ...
       is_type(pc, CT_BRACE_OPEN, CT_BRACE_CLOSE, CT_VBRACE_OPEN, CT_VBRACE_CLOSE))
   {
      // \todo DRY
