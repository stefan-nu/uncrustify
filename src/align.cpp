          ...
          (pc->level >= start->level))
   {
      ...
      if ((next == NULL) || chunk_is_comment(next))
      {
         /* do nothing */
               ...
               (pc->type == CT_COMMA))
      {
         size_t token_width = space_col_align(pc, next); /* \todo can we be sure the result is positive? */
