...
static void flag_asm(chunk_t *pc)
{
   ...
   return_if(!chunk_is_paren_open(po));
...
void do_symbol_check(chunk_t *prev, chunk_t *pc, chunk_t *next)
{
        ...
        is_type(pc, 3, CT_D_CAST, CT_DELEGATE, CT_ALIGN) )
   {
      ...
      if (is_type(pc, CT_DELEGATE))
      {
         ...
         {
            break_if ((chunk_is_semicolon(tmp)    ) ||
...
{
   LOG_FUNC_ENTRY();
   return_if(pc->flags & PCF_IN_PREPROC);
...
static chunk_t *process_return(chunk_t *pc)
{
   ...
   retval_if((is_invalid(next) || chunk_is_semicolon(next)), next);
   ...
   {
      break_if((chunk_is_semicolon(semi) &&
               (pc->level == semi->level)) ||
               (semi->level < pc->level));
   }
   assert(is_valid(semi));
...
//    flag_series(pc, next, (is_type(pc, CT_ENUM)) ? PCF_IN_ENUM : PCF_IN_STRUCT);
   ...
   if (!chunk_is_semicolon(next))
   {
      ...
      return_if(cpd.lang_flags & LANG_PAWN);
          ...
          ((in_fcn_paren ^ (next->flags & PCF_IN_FCN_DEF)) == 0))
   {
      if (next->level == pc->level)
      {
         ...
         return_if(is_type(next, 2, CT_COMMA, CT_FPAREN_CLOSE ) &&
...
static void handle_oc_message_decl(chunk_t *pc)
{
   ...
   while ((tmp = chunk_get_next(tmp)) != nullptr)
   {
      ...
      break_if(is_type(tmp, 2, CT_SEMICOLON, CT_BRACE_OPEN));
   }
