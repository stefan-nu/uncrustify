...
void do_symbol_check(chunk_t *prev, chunk_t *pc, chunk_t *next)
{
   ...
   if (is_type(next, CT_PAREN_OPEN))
   {
      ...
      else if (is_type(pc, CT_TYPE))
      {
         ...
         tmp = get_next_type(next, CT_PAREN_CLOSE, (int)next->level);
         tmp = chunk_get_next(tmp);
       ...
       (is_type(pc, CT_CARET) && (cpd.lang_flags & LANG_CPP)) )
   {
      ...
      else if (is_type(pc, CT_STAR))
      {
         ...
         {
            set_type(pc,(  is_flag(prev, PCF_PUNCTUATOR                         ) &&
...
static bool mark_function_type(chunk_t *pc)
{
   ...
   while ((tmp = chunk_get_prev_ncnl(tmp)) != nullptr)
   {
      ...
      else if (chunk_is_word(tmp) || is_type(tmp, CT_WORD, CT_TYPE))
      {
         word_count++;
   ...
   while (is_valid(next))
   {
               ...
               (is_type(next, CT_OC_COLON) && cs_top_is_question(cs, next->level)) )
      {
         ...
         else
         {
            ...
            {
               tmp = get_next_nc(next, scope_e::PREPROC);
               assert(is_valid(tmp));
