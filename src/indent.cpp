...
void reindent_line(chunk_t *pc, size_t column)
{
   ...
   do
   {
      ...
      bool is_comment = chunk_is_cmt(pc);
      ...
      bool from_caret, bool from_colon,  bool from_keyword)
{
   ...
   tmp = chunk_get_prev_nc(tmp);
   if (is_invalid (tmp                                ) ||
       is_not_type(tmp, CT_OC_MSG_NAME, CT_OC_MSG_FUNC) )
   {
      return(nullptr);
   }
...
void indent_text(void)
{
   ...
   while (is_valid(pc))
   {
      ...
      if (cpd.settings[UO_indent_xml_string].u > 0)
      {
         ...
         {
            if (!chunk_is_cmt(pc) && !chunk_is_nl(pc))
            {
               xml_indent = 0;
            }
         }
      ...
      if (!chunk_is_nl(pc) && !chunk_is_cmt(pc) && log_sev_on(LINDPC))
      {
      ...
      else if (is_type(pc, CT_ASSIGN, CT_IMPORT, CT_USING ) )
      {
         ...
         if (is_valid(next))
         {
            ...
            else if (chunk_is_nl(next) || !cpd.settings[UO_indent_align_assign].b)
            {
               frm.pse[frm.pse_tos].indent = frm.pse[frm.pse_tos - 1].indent_tmp + indent_size;
               if (is_type(pc, CT_ASSIGN))
               {
               ...
               is_ptype(pc, CT_NONE             ) )
      {
         ...
         {
            indent_pse_push(frm, pc);
            if (chunk_is_nl(chunk_get_next(pc)))
            {
               frm.pse[frm.pse_tos].indent  = frm.pse[frm.pse_tos-1].indent + indent_size;
            }
            else
...
bool ifdef_over_whole_file(void)
{
   ...
   {
      continue_if(chunk_is_cmt_or_nl(pc));
