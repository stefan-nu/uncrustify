...
static argval_t do_space(chunk_t *pc1, chunk_t *pc2, int &min_sp, bool complete = true)
{
   ...
   if (is_type(pc2, 2, CT_NEWLINE,
                       CT_VBRACE_OPEN))      { log_argval_and_return(AV_REMOVE); }
   if (is_only_first_type(pc1, CT_VBRACE_OPEN,
                          pc2, CT_NL_CONT))  { log_argval_and_return(AV_FORCE); }
   if (is_only_first_type(pc1, CT_VBRACE_CLOSE,
                          pc2, CT_NL_CONT))  { log_argval_and_return(AV_REMOVE); }
   ...
   if (any_is_type(pc1, pc2,
                   CT_D_ARRAY_COLON))        { log_option_and_return(UO_sp_d_array_colon); }
   ...
   /* handle '::' */
   if (is_type(pc1, CT_DC_MEMBER))
   {
      log_option_and_return(UO_sp_after_dc);
   }
   ...
   if (is_type(pc1, CT_COMMA))
   {
      ...
      {
         log_option_and_return(UO_sp_after_comma);
      }
   }
   ...
   if (is_type(pc1, CT_ELLIPSIS) &&
       CharTable::IsKW1((size_t)(pc2->str[0]))) { log_argval_and_return(AV_FORCE); }
   if (is_type(pc1, CT_TAG_COLON))
   {
      log_option_and_return(UO_sp_after_tag);
   }
   if (is_type(pc2, CT_TAG_COLON)) { log_argval_and_return(AV_REMOVE); }

   /* handle '~' */
   if (is_type(pc1, CT_DESTRUCTOR)) { log_argval_and_return(AV_REMOVE); }
   ...
   /* "if (" vs "if(" */
   if (is_type(pc2, CT_SPAREN_OPEN))
   {
      log_option_and_return(UO_sp_before_sparen);
   }

   if (any_is_type(pc1, pc2, CT_LAMBDA))
   {
      log_option_and_return(UO_sp_assign);
   }
   ...
   if (is_type(pc2, CT_ASSIGN))
   {
      if (pc2->flags & PCF_IN_ENUM)
      {
         ...
         log_option_and_return(UO_sp_enum_assign);
      }
      if ((is_not_option(cpd.settings[UO_sp_assign_default].a, AV_IGNORE)) &&
          is_ptype(pc2, CT_FUNC_PROTO))
      {
         log_option_and_return(UO_sp_assign_default);
      }
   ...
   if (any_is_type(pc1, CT_ANGLE_OPEN, pc2, CT_ANGLE_CLOSE))
   {
   ...
   if (is_type(pc1, CT_ANGLE_CLOSE))
   {
      ...
      if (is_not_type(pc2, 2, CT_BYREF, CT_PTR_TYPE))
      {
   ...
   if (is_type(pc1, 2, CT_CPP_CAST, CT_TYPE_WRAP))
   {
      log_rule("sp_cpp_cast_paren");
      return(cpd.settings[UO_sp_cpp_cast_paren].a);
   }

   if (are_types(pc1, CT_PAREN_CLOSE, pc2, CT_WHEN))
   {
      log_argval_and_return(AV_FORCE); /* TODO: make this configurable? */
   }
   ...
   if (is_type(pc1, CT_TPAREN_CLOSE))
   {
   ...
   if (are_types(pc1, CT_PP_DEFINED, pc2, CT_PAREN_OPEN))
   {
   ...
   if (are_types(pc1, CT_STATE, pc2, CT_PAREN_OPEN)) { log_argval_and_return(AV_ADD); }
   if (are_types(pc1, CT_DELEGATE, pc2, CT_PAREN_OPEN)) { log_argval_and_return(AV_REMOVE); }
   ...
   if (any_is_type(pc1, pc2, CT_D_TEMPLATE)) { log_argval_and_return(AV_REMOVE); }
   ...
   if (are_types(pc1, CT_ELSE, pc2, CT_ELSEIF)) { log_argval_and_return(AV_FORCE); }
   ...
   if (is_type(pc1, CT_PAREN_CLOSE))
   {
      ...
      if (is_type(pc2, 2, CT_ARITH, CT_CARET))
      {
         log_rule("sp_arith");
   ...
   /* "foo(...)" vs "foo( ... )" */
   if (any_is_type(pc1, CT_FPAREN_OPEN, pc2, CT_FPAREN_CLOSE))
   {
   ...
   if (any_is_type(pc1, pc2, CT_NULLCOND))
   {
      log_rule("sp_member");
      return(cpd.settings[UO_sp_member].a);
   }
   ...
   if (is_type(pc1,  2, CT_ARITH, CT_CARET) ||
       is_type(pc2, 2, CT_ARITH, CT_CARET) )
   {
      log_rule("sp_arith");
      return(cpd.settings[UO_sp_arith].a);
   }
   ...
   if (any_is_type(pc1, pc2, CT_COMPARE))
   {
      log_rule("sp_compare");
      return(cpd.settings[UO_sp_compare].a);
   }

   if (are_types(pc1, CT_PAREN_OPEN, pc2, CT_PTR_TYPE)) { log_argval_and_return(AV_REMOVE); }
   ...
   if (any_is_type(pc1, pc2, CT_PTR_TYPE))
   {
      log_argval_and_return(AV_IGNORE);
   }

   if (is_type(pc1, CT_NOT))
   {
      log_rule("sp_not");
      return(cpd.settings[UO_sp_not].a);
   }
   if (is_type(pc1, CT_INV))
   {
      log_rule("sp_inv");
      return(cpd.settings[UO_sp_inv].a);
   }
   if (is_type(pc1, CT_ADDR))
   {
      log_rule("sp_addr");
      return(cpd.settings[UO_sp_addr].a);
   }
   if (is_type(pc1, CT_DEREF))
   {
      log_rule("sp_deref");
      return(cpd.settings[UO_sp_deref].a);
   }
   if (is_type(pc1, 2, CT_POS, CT_NEG) )
   {
      log_rule("sp_sign");
   ...
   if (is_type(pc1, CT_NEW       ) &&
       is_type(pc2, CT_PAREN_OPEN) )
   {
